...
def insert_db_using_sql(connection, table_name, data_field, list_data):
    ...
    data_insert = [f"""({idx} ,'{item['movie_link']}', '{item['poster_image_link']}','{item['movie_name']}','{item['movie_status']}','{item['en_name']}','{item['vn_name']}')""" for idx, item in enumerate(list_data)]
...
def insert_db_using_copy_string_iterator(connection, tbl_name, data_field, iter_data: Iterator[Dict[str, Any]], size: int = 8192) -> None:
